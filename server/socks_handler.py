...
class TunnelHandler:
    ...
    _dns_futures: Dict[str, asyncio.Future] = {}   # in-flight DNS requests (предотвращает Thundering Herd)
    ...
    async def _async_resolve_host(cls, host: str) -> str:
        """Сверхбыстрый асинхронный резолвер DNS с зашитой от шторма запросов."""
        try:
        ...
        # Если этот же домен уже резолвится прямо сейчас в параллельном таске — ждем его результат
        ...
        try:
            ...
            cls._dns_cache[host] = (resolved_ip, now + 300.0)  # Кэш на 5 минут
